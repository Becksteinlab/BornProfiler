 ...
 * NOTE: this program was changed in 2005 to     *
 ...
 * NOTE: this program was changed in 2010 to     *
 * also allow for polar head group.              *
 *                                               * 
 * INPUTS:                                       *
 ...
 * mdie   - membrane dielectric constant         *
 * idie   - lipid headgroup dielectric constant  *
...
/*****************************************************************************/
/* INPUT LOOKS LIKE:                                                         */
/* mgrabe% draw_mem  dielx*.dx z_m0 l_m pdie sdie mdie idie V I R_m1 R_m0 a0 */
/*****************************************************************************/
...
pdie=atof(argv[4]);
sdie=atof(argv[5]);  	/* solvent dielectric set from commandline */
mdie=atof(argv[6]);  	/* ditto membrane di, this is different from version2 */
idie=atof(argv[7]); 	/* headgroup dielectric set from commandline, different from version3 */
...
a0=atof(argv[12]);	/* headgroup thickness set from commandline, different from version3 */
	...
	{	
		for (i=1; i <= dim_z; ++i)  /* loop over x */
	        {	
		        ...
		        {
				if (z_x[i] <= au && z_x[i] >= ad ) {	
                	        	d_x[cnt] = mdie;   /* bilayer dielectric constant */
				}
				else {
					d_x[cnt] = idie;   /* headgroup dielectric */
				}	
                        }
                        ...
                        {      
                        	if (z_y[i] <= au && z_y[i] >= ad ) {	
                	        	d_y[cnt] = mdie;   /* bilayer dielectric constant */
				}
				else {
					d_y[cnt] = idie;   /* headgroup dielectric */
				}
                        }
                        ...
                        {      
                        	if (z_z[i] <= au && z_z[i] >= ad ) {	
                	        	d_z[cnt] = mdie;   /* bilayer dielectric constant */
				}
				else {
					d_z[cnt] = idie;   /* headgroup dielectric */
				}
                        }
